#!/usr/bin/env python3
"""Experiment C: Train with roi198 fine parcellation for intensity sampling, roi22 targets."""
...
def discover_data(cfg: dict):
    ...
    return (
        [fine_dict[s] for s in train_subs],    # fine labels for synth
        [coarse_dict[s] for s in train_subs],   # coarse labels for targets
        [coarse_dict[s] for s in val_subs],      # val coarse labels
        [image_dict[s] for s in val_subs],        # val images
    )
...
def main():
    ...
    parser.add_argument("--config", default="configs/experiment_c_roi198.yaml")
    ...
    print(f"[Experiment C - roi198] GPU {args.gpu}: {torch.cuda.get_device_name(device)}")
    ...
    print(f"\n{'='*60}")
    print(f"Experiment C: roi198 fine parcellation (198 labels for intensity)")
    print(f"  Target classes: {n_labels} (roi22), Fine labels: 198")
    ...
    for epoch in range(start_epoch, train_cfg["epochs"]):
        t0 = time.time()
        epoch_losses = []
        pbar = tqdm(range(steps_per_epoch), desc=f"ExpC Epoch {epoch}", leave=False)
        for step in pbar:
        ...
        if (epoch + 1) % train_cfg["val_every"] == 0:
            ...
            print(f"ExpC Epoch {epoch:4d} | step {global_step} | "
    ...
    save_checkpoint(model, optimizer, train_cfg["epochs"] - 1, global_step, best_dice,
                    ckpt_dir / "final.pt")
    print(f"\n[Experiment C] Done. Best val dice: {best_dice:.4f}")
