#!/usr/bin/env python3
"""Experiment F: roi-all fine parcellation + full SynthSeg-faithful augmentation."""
...
def main():
    ...
    parser.add_argument("--config", default="configs/experiment_f_roiall_augv2.yaml")
    ...
    print(f"[Experiment F] GPU {args.gpu}: {torch.cuda.get_device_name(device)}")
    ...
    print(f"\n{'='*60}")
    print(f"Experiment F: roi-all (~1339 labels) + full SynthSeg augmentation")
    ...
    print(f"  Fine labels for intensity: ~1339, target: {n_labels} (roi22)")
    ...
    for epoch in range(start_epoch, train_cfg["epochs"]):
        t0 = time.time()
        epoch_losses = []
        pbar = tqdm(range(steps_per_epoch), desc=f"ExpF Epoch {epoch}", leave=False)
        for step in pbar:
        ...
        if (epoch + 1) % train_cfg["val_every"] == 0:
            ...
            print(f"ExpF Epoch {epoch:4d} | step {global_step} | "
    ...
    save_checkpoint(model, optimizer, train_cfg["epochs"] - 1, global_step, best_dice,
                    ckpt_dir / "final.pt")
    print(f"\n[Experiment F] Done. Best val dice: {best_dice:.4f}")
