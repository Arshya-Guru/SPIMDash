#!/usr/bin/env python3
"""
train_b.py — Experiment B: SPIM-aware probabilistic augmentation.

Identical to train.py except uses gpu_synth_v2 and defaults to experiment_b config.
"""
...
from utils.gpu_synth_v2 import GPUSynthGeneratorV2
...
def main():
    ...
    parser.add_argument("--config", default="configs/experiment_b.yaml")
    ...
    print(f"[Experiment B] Using GPU {args.gpu}: {torch.cuda.get_device_name(device)}")
    ...
    print(f"\n{'='*60}")
    print(f"Experiment B: SPIM-aware probabilistic augmentation")
    ...
    print(f"  Augmentation:  affine+elastic(50%), bias(80%), gamma(50%)")
    print(f"  Intensity:     background-aware (bg=low, brain=high)")
    print(f"  Smoothing:     mandatory boundary smooth [0.5, 2.0]")
    print(f"{'='*60}\n")
    ...
    for epoch in range(start_epoch, train_cfg["epochs"]):
        ...
        pbar = tqdm(range(steps_per_epoch), desc=f"ExpB Epoch {epoch}", leave=False)
        for step in pbar:
        ...
        if (epoch + 1) % train_cfg["val_every"] == 0:
            ...
            print(f"ExpB Epoch {epoch:4d} | step {global_step} | "
    ...
    save_checkpoint(model, optimizer, train_cfg["epochs"] - 1, global_step, best_dice,
                    ckpt_dir / "final.pt")
    print(f"\n[Experiment B] Done. Best val dice: {best_dice:.4f}")
